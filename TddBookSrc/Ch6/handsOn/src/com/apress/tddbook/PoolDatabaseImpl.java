...
public class PoolDatabaseImpl implements PoolDatabase
{
    ...
    public Vector getPoolWithStatus(String status) throws NoPoolFoundException
    {
        ...
        try
        {
            ...
            //If no results found throw execption so that client knows that no pool was found
            if (!morePoolResults)
            {
                throw new NoPoolFoundException();
            }
    ...
    public void savePlayersPicks(PlayersPicks picks) throws SavePlayerPickException
    {
        ...
        try
        {
            ...
            if (!morePoolResults)
            {
                throw new SavePlayerPickException("No pool found for date " + poolDate);
            }
            if (morePoolResults)
            {
                poolID = poolTBResults.getInt(1);
        ...
        try
        {
            ...
            if (morePoolResults)
            {
                ...
                for (int i = 0; i < 14; i++)
                {
                    ...
                    //If no picks found throw execption so that client knows that no picks were found
                    if (!morePickResults)
                    {
